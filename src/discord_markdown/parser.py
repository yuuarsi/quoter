        ...
        while current_token != self.eof:
            ...
            if current_token.type in FORMAT_TOKEN_TYPES:
                ...
                if current_token.type != TokenSpecification.NEWLINE.name:
                    if current_token.type == TokenSpecification.TEXT.name:
                        node = AST_BY_TOKEN_TYPE[TokenSpecification.TEXT.name](
                            current_token.value
                        )
                    elif current_token.type == TokenSpecification.DISCORD_EMOJI.name:
                        node = AST_BY_TOKEN_TYPE[TokenSpecification.DISCORD_EMOJI.name](
                            current_token.value
                        )
                    elems.append(node)
